...
import { getGadgets } from '@/services/gadgets/gadgets'
import GadgetsTree from '@/components/gadgets/tree/GadgetsTree'
...
export default {
  ...
  methods: {
    setFocus () {
      this.$nextTick(() => {
        setTimeout(() => { this.$refs['gadgetTab-' + this.activeTab][0].$el.focus() }, 2000)
      })
    },
    async getGadgets () {
      try {
        const gadgets = await getGadgets({ group: 'custom' })
        this.gadgets = gadgets.filter(
          (g) => g.type === 'vueJSODS' || g.type === 'vueJS'
        )
      } finally {
        this.loading = false
      }
