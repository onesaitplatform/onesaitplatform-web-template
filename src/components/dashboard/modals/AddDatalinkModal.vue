  ...
  >
    ...
    <ods-module :header="$t('addDatalink.addConnections')" :body-style="{'padding':'12px'}" style="margin: 24px auto">
        <div class="datalink-form">
            <div>
              <!-- gadget source -->
              <ods-select  v-model="emitterGadget" :multiple="false" :disabled="false"  :collapse-tags="false" :size="'default'"  :clearable="true" :placeholder="'select source gadget'" :filterable="true" :allow-create="true"  @change="refreshGadgetEmitterFields(emitterGadget)">
            ...
            <div>
              <!-- gadget target -->
              <ods-select  v-model="targetGadget" :multiple="false" :disabled="false"  :collapse-tags="false" :size="'default'"  :clearable="true" :placeholder="'select target gadget'" :filterable="true" :allow-create="true"  @change="refreshGadgetTargetFields(targetGadget)">
    ...
    <ods-module :header="$t('addDatalink.showConnections')" :body-style="{'padding':'12px'}">
        <div class="ods-flex">
            <ods-table ref="connectionsTable"
            :data="connections"
                ...
                <template slot-scope="scope">
                    <ods-tooltip class="item" effect="dark" :content="'Edit ' + scope.row.source" placement="top-start">
                        <ods-button size="small" type="secondary" @click="edit(scope.row)">
                            <i class="ods-icon-edit" style="font-size: .9rem;"></i>
                        </ods-button>
                    </ods-tooltip>
                    <ods-tooltip class="item" effect="dark" :content="'Remove Connection' + scope.row.source" placement="top-start">
                        <ods-button size="small" type="secondary"
                            @click="remove(scope.row)"><i class="ods-icon-delete" style="font-size: .9rem;"></i>
                        </ods-button>
                    </ods-tooltip>
                </template>
            </ods-table-column>
