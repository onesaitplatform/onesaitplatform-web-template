...
import { getResourcesByTag } from '@/services/tags/tags'
...
export const getOntologies = async (config = {}) => {
  ...
  var { data: projectOntologies } = await api.platform.get(`/projects/${config.project}/resources/role/${config.role}`, {
    ...config,
    notify
  })
  console.log('RESOURCES FROM ONTOLOGIES: ', ontologies)
  // adjust to ontologies and add identification key
  projectOntologies = projectOntologies.length > 0 ? projectOntologies.filter(x => x.resourceType === 'ONTOLOGY').map((y) => ({ ...y, identification: y.resource })) : []
